...
async fn cmd_status(json: bool) -> anyhow::Result<()> {
    ...
    if json {
        ...
        let dir = daemon::talkd_dir();
        let contacts = crypto::load_contacts(&dir);
        // Build reverse lookup: short_id → contact name
        let short_to_name: std::collections::HashMap<String, String> = contacts
            .iter()
            .map(|(name, c)| (c.id.fmt_short().to_string(), name.clone()))
            .collect();
        ...
        if channels.is_empty() {
            ...
            // Separate channels with new messages from quiet ones
            let mut active: Vec<_> = channels.iter().filter(|(_, i)| i.buffered > 0).collect();
            let mut quiet: Vec<_> = channels.iter().filter(|(_, i)| i.buffered == 0).collect();
            active.sort_by(|(a, _), (b, _)| a.cmp(b));
            quiet.sort_by(|(a, _), (b, _)| a.cmp(b));

            let resolve_name = |name: &str| -> String {
                if let Some(short_id) = name.strip_prefix("dm:") {
                    if let Some(contact) = short_to_name.get(short_id) {
                        return format!("dm:{}", contact);
                    }
                }
                name.to_string()
            };
            ...
            if !active.is_empty() {
                ...
                for (name, info) in &active {
                    let display_name = resolve_name(name);
                    println!("  #{} ({} new)", display_name, info.buffered);

                    // Show preview only for channels with new messages
                    for msg in &info.preview {
                        let from = if let Some(contact) = short_to_name.get(&msg.from) {
                            contact.as_str()
                        } else if msg.from == "default" {
                            "you"
                        } else {
                            &msg.from
                        };
                        let text = if msg.data.len() > 60 {
                            format!("{}...", &msg.data[..57])
                        } else {
                            msg.data.clone()
                        };
                        let file_indicator = msg.file.as_ref().map_or("", |_| " [file]");
                        println!("    {}: {}{}", from, text, file_indicator);
                    }
                    println!();
                }
            }
            ...
            if !quiet.is_empty() {
                if active.is_empty() { println!(); }
                let names: Vec<String> = quiet.iter()
                    .map(|(name, info)| {
                        let dn = resolve_name(name);
                        if info.total > 0 {
                            format!("#{} ({})", dn, info.total)
                        } else {
                            format!("#{}", dn)
                        }
                    })
